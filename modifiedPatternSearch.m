function [bestOpt, bestOdf] = modifiedPatternSearch(divider)

fhandle =@SeparateOptY;
fileName = strcat('modifiedPatternDat',num2str(0),'_',num2str(divider),'.mat');
...
for j = 1: 76 % for each variable
    ...
    clear temp*
    myStep = (1/constraint(j))/divider;
    searchRangeWide = 0:myStep:1/constraint(j); 
    for i = 1:length(searchRangeWide)
        ...
        [odf,opt] = patternsearch(fhandle,odf0,[],[],constraint,1,zeros(1,76),ub,@simpleConstraint);
    ...
    elseif ismember(length(searchRangeWide),tempInd) % one of the best is at the end
        ...
        for rs_iter = 1: length(tempInd)
            ...
            % re-search
	    mySmallStep = (searchRangeWide(temp_IndEach+1) - searchRangeWide(temp_IndEach-1))/divider;
            searchRangeNarrow = searchRangeWide(temp_IndEach-1):mySmallStep:searchRangeWide(temp_IndEach+1);
