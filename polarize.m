...
%if isempty(strfind(type, 'smalGl')) == 0
...
veryLowTarData = data(ind(1:dsizeLow),:);
%lowLower = floor(0.5*dsizeLow);
%lowUpper = floor(1.5*dsizeLow);
%lowTarData = data(ind(lowLower:lowUpper-1),:);

lowTarData = veryLowTarData;
...
normLowData = [normLowData,zeros(dsizeLow,1)];
...
fileName = strcat('Weka/NoShifted',num2str(percent),'_',num2str(Var),'.csv');
