function [bestOpt, bestOdf] = modPatternSearch(bigStep,smallStep)

fhandle =@SeparateOptY;
fileName = strcat('modPatternDat',num2str(bigStep),'_',num2str(smallStep),'.mat');
...
for j = 1: 76 % for each variable
    ...
    elseif ismember(length(searchRangeWide),tempInd) % one of the best is at the end
        ...
        for rs_iter = 1: length(tempInd)
            ...
            % re-search
	    %mySmallStep = (searchRangeWide(temp_IndEach+1) - searchRangeWide(temp_IndEach-1))/10;
